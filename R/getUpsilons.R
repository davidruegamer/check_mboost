#' Compute residual matrix for boosting object (originally implemented in the \code{iboost} package)
#'
...
#' @return Returns a list of length \code{mstop(mod)+1} with residual matrices of size \code{length(mod$response)}.
#' @examples
  ...
  )
{
  ...
  if(any(sapply(hatMats,is.null)) | !returnUpsOnly | (any(w!=1)) | inherits(mod, "glmboost")){
    ...
    if(inherits(mod,"glmboost")){
      ...
      if(any(lambdas!=0) & any(w!=1)) 
        stop("Can't deal with given weights and penalized baselearners.")
      
      pens <- lapply(1:length(mod$baselearner), function(i) 
        extract(mod, what="penalty", which=i)[[1]])
  ...
  if(returnUpsOnly) return(Upsilons) else 
    return(list(Upsilons=Upsilons, hatMats=hatMats, hatMatsLeft=hatMatsLeft))
